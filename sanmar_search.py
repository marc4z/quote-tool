...
# Thread-local storage for per-request SanMar credentials
...
# ─── Per-Request Credential Helper ──────────────────────────────────────────

def get_request_creds():
    """Return (username, password, customer_id) for the current request.
    Priority: thread-local (set by route handlers before spawning threads)
              → X-SM-User / X-SM-Pwd / X-SM-Cust request headers
              → CONFIG environment / defaults.
    """
    # Thread-local takes top priority (used when running inside ThreadPoolExecutor)
    tl = _creds_local
    if getattr(tl, 'username', None):
        return tl.username, tl.password, tl.customer_id

    # Try request context (may not be available in background threads)
    try:
        u = request.headers.get('X-SM-User', '').strip()
        p = request.headers.get('X-SM-Pwd', '').strip()
        c = request.headers.get('X-SM-Cust', '').strip()
        if u and p:
            return u, p, c or CONFIG['customer_id']
    except RuntimeError:
        pass  # No active request context (background thread)

    return CONFIG['username'], CONFIG['password'], CONFIG['customer_id']


def set_thread_creds(username, password, customer_id):
    """Store creds in thread-local so they survive ThreadPoolExecutor dispatch."""
    _creds_local.username = username
    _creds_local.password = password
    _creds_local.customer_id = customer_id


def clear_thread_creds():
    """Reset thread-local creds after request is done."""
    _creds_local.username = None
    _creds_local.password = None
    _creds_local.customer_id = None
...
def fetch_product_full(product_id):
    ...
    # Capture creds from the current request context BEFORE spawning threads
    # (the request context is not available inside ThreadPoolExecutor workers)
    try:
        u = request.headers.get('X-SM-User', '').strip() or CONFIG['username']
        p = request.headers.get('X-SM-Pwd', '').strip() or CONFIG['password']
        c = request.headers.get('X-SM-Cust', '').strip() or CONFIG['customer_id']
    except RuntimeError:
        u, p, c = CONFIG['username'], CONFIG['password'], CONFIG['customer_id']

    def _with_creds(fn, *args):
        """Wrapper that sets thread-local creds before calling fn."""
        set_thread_creds(u, p, c)
        try:
            return fn(*args)
        finally:
            clear_thread_creds()
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {
            executor.submit(_with_creds, fetch_product_details, product_id): 'product',
            executor.submit(_with_creds, fetch_inventory, product_id): 'inventory',
            executor.submit(_with_creds, fetch_media, product_id): 'media',
            executor.submit(_with_creds, fetch_pricing, product_id): 'pricing',
        }
        for future in as_completed(futures):
            key = futures[future]
            try:
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .swatch.no-local-stock {
    opacity: 0.38;
    text-decoration: line-through;
    cursor: default;
  }
  .swatch.no-local-stock:hover {
  ...
  .creds-overlay {
    display: none; position: fixed; inset: 0;
    background: rgba(10,20,40,.72); z-index: 2000;
    align-items: center; justify-content: center; padding: 20px;
  }
  .creds-overlay.open { display: flex; }
  .creds-modal {
  ...
  .creds-logo svg { flex-shrink: 0; }
  .creds-title { font-size: 1.25rem; font-weight: 800; color: #1a3a5c; }
  .creds-sub { font-size: .85rem; color: #64748b; margin-top: 2px; }
  ...
  .creds-input {
    width: 100%; padding: 11px 14px; border: 1.5px solid #dde1e7;
    border-radius: 8px; font-size: .95rem; outline: none; transition: border .2s;
  }
  .creds-input:focus { border-color: #1a3a5c; }
  .creds-save-btn {
    width: 100%; padding: 13px; background: #1a3a5c; color: #fff;
    border: none; border-radius: 10px; font-size: 1rem; font-weight: 700;
    cursor: pointer; transition: background .2s; margin-top: 6px;
  }
  .creds-save-btn:hover { background: #2a5a8c; }
  .creds-note { font-size: .76rem; color: #94a3b8; text-align: center; margin-top: 12px; }
  .creds-status {
    font-size: .82rem; text-align: center; margin-top: 10px;
    font-weight: 700; min-height: 20px;
  }
  .creds-status.ok { color: #16a34a; }
  .creds-status.err { color: #dc2626; }
  .creds-gear-btn {
    background: rgba(255,255,255,.15); border: none; border-radius: 8px;
    color: white; cursor: pointer; padding: 8px 10px; font-size: 18px;
    transition: background .2s; line-height: 1;
  }
  .creds-gear-btn:hover { background: rgba(255,255,255,.28); }
  .creds-badge {
  ...
  .creds-badge.set { background: rgba(34,197,94,.2); color: #4ade80; }
  .creds-badge.unset { background: rgba(239,68,68,.2); color: #f87171; animation: pulse-warn .8s infinite alternate; }
  @keyframes pulse-warn { from { opacity: .7; } to { opacity: 1; } }
</style>
</head>
<body>

<!-- ── SanMar Credentials Modal ── -->
<div class="creds-overlay" id="credsOverlay">
  <div class="creds-modal">
    <div class="creds-logo">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 405 430" width="40" height="40">
        <path fill="#1a3a5c" d="M221.8,387.7c21.6,0,42.2-4.4,60.9-12.3h52c-31.1,24.4-70.3,39-113,39-101.1,0-183-81.9-183-183s0-3,0-4.5h26.7c0,1.5,0,3,0,4.5,0,86.3,70,156.3,156.3,156.3h0ZM38.7,208.7h120.3v88.7l42-43.1v-45.6h79.9l-123.6,127.2,15.1,21.3h182.2c31.1-32.8,50.2-77.1,50.2-125.9,0-101.1-81.9-183-183-183s-1.8,0-2.7,0v26.7c.9,0,1.8,0,2.7,0,86.3,0,156.3,70,156.3,156.3s-9.7,61.9-26.3,86.7h-113.7l106.9-109.4v-39.2h-144.1V48.6h-42l-120.3,120.9v39.2h0ZM159,102.1v67.4h-65.9l65.9-67.4h0Z"/>
      </svg>
      <div>
        <div class="creds-title">SanMar API Credentials</div>
        <div class="creds-sub">Enter your SanMar account info to get started</div>
      </div>
    </div>
    <form id="credsForm" autocomplete="on" onsubmit="saveCreds(); return false;">
      <div class="creds-field">
        <label class="creds-label" for="credsUser">Username</label>
        <input class="creds-input" type="text" id="credsUser" name="username"
               autocomplete="username" placeholder="Your SanMar username" required />
      </div>
      <div class="creds-field">
        <label class="creds-label" for="credsPwd">Password</label>
        <input class="creds-input" type="password" id="credsPwd" name="password"
               autocomplete="current-password" placeholder="••••••••" required />
      </div>
      <div class="creds-field">
        <label class="creds-label" for="credsCust">Customer ID</label>
        <input class="creds-input" type="text" id="credsCust" name="customer-id"
               autocomplete="off" placeholder="e.g. 140741" />
      </div>
      <button type="submit" class="creds-save-btn" id="credsSaveBtn">Save &amp; Connect</button>
    </form>
    <div class="creds-status" id="credsStatus"></div>
    <div class="creds-note">Credentials are stored only in this browser. Never sent anywhere except SanMar.</div>
  </div>
</div>
...
<header class="header">
  <div class="header-inner">
    ...
    <button class="creds-gear-btn" onclick="openCredsModal()" title="SanMar API Credentials" id="credsGearBtn">&#9881;</button>
    <button class="creds-badge unset" id="credsBadge" onclick="openCredsModal()" style="display:none;"></button>
  </div>
...
function loadCreds() {
  return {
    ...
    pwd:  localStorage.getItem('sm_pwd')  || '',
    cust: localStorage.getItem('sm_cust') || '',
  };
}

function credsAreSet() {
  const c = loadCreds();
  return !!(c.user && c.pwd);
}
...
function updateCredsBadge() {
  ...
  const gear  = document.getElementById('credsGearBtn');
  if (!badge) return;
  if (credsAreSet()) {
    const c = loadCreds();
    badge.className = 'creds-badge set';
    badge.textContent = '✓ ' + c.user;
    badge.style.display = '';
    if (gear) gear.style.display = 'none';
  } else {
    badge.className = 'creds-badge unset';
    badge.textContent = '⚠ credentials needed';
    badge.style.display = '';
    if (gear) gear.style.display = 'none';
  }
}

function openCredsModal(forceFocus) {
  const c = loadCreds();
  document.getElementById('credsUser').value = c.user;
  document.getElementById('credsPwd').value  = c.pwd;
  document.getElementById('credsCust').value = c.cust;
  document.getElementById('credsStatus').textContent = '';
  document.getElementById('credsStatus').className = 'creds-status';
  document.getElementById('credsOverlay').classList.add('open');
  setTimeout(() => document.getElementById('credsUser').focus(), 80);
}

function closeCredsModal() {
  document.getElementById('credsOverlay').classList.remove('open');
}

async function saveCreds() {
  const u = document.getElementById('credsUser').value.trim();
  const p = document.getElementById('credsPwd').value.trim();
  const c = document.getElementById('credsCust').value.trim();
  const status = document.getElementById('credsStatus');
  const btn = document.getElementById('credsSaveBtn');

  if (!u || !p) { status.textContent = 'Username and password are required.'; status.className = 'creds-status err'; return; }

  btn.textContent = 'Verifying...'; btn.disabled = true;
  status.textContent = ''; status.className = 'creds-status';

  // Quick healthcheck to validate creds
  try {
    const resp = await smFetchRaw('/api/healthcheck', {}, u, p, c);
    const data = await resp.json();
    const anyOk = Object.values(data).some(v => v && v.ok);
    if (!anyOk && resp.ok) {
      status.textContent = 'Saved — but API check shows issues. Verify your credentials.';
      status.className = 'creds-status err';
    } else {
      status.textContent = '✓ Connected to SanMar!';
      status.className = 'creds-status ok';
    }
  } catch(e) {
    status.textContent = 'Saved (could not verify — check your connection).';
    status.className = 'creds-status err';
  }

  localStorage.setItem('sm_user', u);
  localStorage.setItem('sm_pwd',  p);
  localStorage.setItem('sm_cust', c);

  btn.textContent = 'Save & Connect'; btn.disabled = false;
  updateCredsBadge();

  setTimeout(() => { if (credsAreSet()) closeCredsModal(); }, 1800);
}

// smFetchRaw: inject creds headers, optionally override creds
function smFetchRaw(url, options = {}, overrideUser, overridePwd, overrideCust) {
  const c = loadCreds();
  const u = overrideUser !== undefined ? overrideUser : c.user;
  const p = overridePwd  !== undefined ? overridePwd  : c.pwd;
  const ct = overrideCust !== undefined ? overrideCust : c.cust;
  const headers = Object.assign({}, options.headers || {});
  if (u) headers['X-SM-User'] = u;
  if (p) headers['X-SM-Pwd']  = p;
  if (ct) headers['X-SM-Cust'] = ct;
  return fetch(url, { ...options, headers });
}

// smFetch: standard wrapper used everywhere in this app
async function smFetch(url, options = {}) {
  if (!credsAreSet()) { openCredsModal(); throw new Error('No credentials set'); }
  return smFetchRaw(url, options);
}

// Dismiss modal on overlay click
document.getElementById('credsOverlay').addEventListener('click', function(e) {
  if (e.target === this && credsAreSet()) closeCredsModal();
});
...
async function doSearch() {
  ...
  try {
    const resp = await smFetch(`/api/search?q=${encodeURIComponent(q)}`);
    const data = await resp.json();
...
async function loadProduct(productId) {
  showLoading();
  try {
    const resp = await smFetch(`/api/product/${encodeURIComponent(productId)}`);
    const data = await resp.json();
    if (data.error) {
      showError(data.error);
      return;
...
function renderProductCard(prod) {
  ...
  // Determine which colors have stock at the selected warehouses
  const inStockColorNames = new Set();
  const inv = prod.inventory;
  if (inv && inv.parts && selectedWarehouses.size > 0) {
    for (const part of inv.parts) {
      const localQty = (part.locations || [])
        .filter(loc => selectedWarehouses.has(loc.name))
        .reduce((sum, loc) => sum + parseInt(loc.qty || 0), 0);
      if (localQty > 0) {
        inStockColorNames.add((part.partColor || '').toLowerCase().trim());
      }
    }
  }

  // Helper: does a color name match any in-stock part color?
  function isColorInStock(colorName) {
    if (inStockColorNames.size === 0) return true; // no inventory data — show all
    const cn = colorName.toLowerCase().trim();
    for (const ic of inStockColorNames) {
      if (ic.includes(cn) || cn.includes(ic)) return true;
    }
    return false;
  }

  // Pick default color: active filter first, then first in-stock, then first overall
  let defaultColor = selectedColor || null;
  if (!defaultColor && prod.allColors && prod.allColors.length > 0) {
    for (const c of prod.allColors) {
      if (isColorInStock(c.name)) { defaultColor = c.name; break; }
    }
    if (!defaultColor) defaultColor = prod.allColors[0].name; // all OOS — still pick something
  }

  // Pick default image matching the default color
  let defaultImageUrl = '';
  if (prod.images && prod.images.length > 0) {
    defaultImageUrl = prod.images[0].url;
    const colorToMatch = defaultColor;
    if (colorToMatch) {
      const colorLower = colorToMatch.toLowerCase();
  ...
  if (prod.allColors && prod.allColors.length > 0) {
    // Sort: in-stock colors first, out-of-stock last
    const sortedColors = [...prod.allColors].sort((a, b) => {
      const aStock = isColorInStock(a.name);
      const bStock = isColorInStock(b.name);
    ...
    for (const c of sortedColors) {
      const inStock = isColorInStock(c.name);
      const isActive = defaultColor && c.name === defaultColor;
      const noStockClass = (!inStock && inStockColorNames.size > 0) ? 'no-local-stock' : '';
      const title = c.name + (!inStock && inStockColorNames.size > 0 ? ' — not in local warehouse' : '');
      const clickHandler = inStock ? `onclick="switchImage('${pid}', '${c.name.replace(/'/g, "\\'")}')"` : '';
      swatchesHtml += `<span class="swatch ${isActive ? 'active' : ''} ${noStockClass}" title="${title}" ${clickHandler}>${c.name}</span>`;
    }
    swatchesHtml += '</div>';
  }
...
async function buildCatalog() {
  ...
  try {
    await smFetch('/api/catalog/build', {method: 'POST'});
    pollCatalogStatus();
  } catch(err) {
...
async function pollCatalogStatus() {
  try {
    const resp = await smFetchRaw('/api/catalog/status');
    const data = await resp.json();
    const btn = document.getElementById('catalogBtn');
...
async function runHealthCheck() {
  showLoading();
  try {
    const resp = await smFetch('/api/healthcheck');
    const data = await resp.json();
...
(function initPage() {
  updateCredsBadge();
  if (!credsAreSet()) {
    // Show modal on first visit instead of landing on a broken search experience
    openCredsModal();
  }
})();
</script>
